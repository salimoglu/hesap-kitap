undefinedundefined/* yukle.js - ButceModule + KrediModule */
...
/* Fonksiyonları global scope'a expose et */
  window.guncelAltinCek=guncelAltinCek;
  window.afbFiyatKaydet=afbFiyatKaydet;
  window.arender=arender;
  window._getGuncelGramFiyat=function(){return _guncelGramFiyat;};
  window._setGuncelGramFiyat=function(v){_guncelGramFiyat=v;};
  return{init:ainit};
})();
...
function abagla(){
  ...
  $("alt-kaydet").addEventListener("click",akaydet);
  /* Fiyat güncelle */
  /* Güncelle butonu — document delegation ile render'dan bağımsız */
  if(!window._altinBtnListenerAdded) {
    window._altinBtnListenerAdded = true;
    document.addEventListener('click', async function(e) {
      const btn = e.target.closest ? e.target.closest('#alt-fiyat-guncelle') : null;
      const target = btn || (e.target.id === 'alt-fiyat-guncelle' ? e.target : null);
      if(!target) return;
      if(target._updating) return;
      target._updating = true;
      target.style.animation = 'spin 1s linear infinite';
      target.disabled = true;
      var f = await guncelAltinCek();
      var target2 = document.getElementById('alt-fiyat-guncelle');
      if(target2) { target2.style.animation = ''; target2.disabled = false; target2._updating = false; }
      if(f > 0) {
        _guncelGramFiyat = f;
        await afbFiyatKaydet(f);
        arender();
