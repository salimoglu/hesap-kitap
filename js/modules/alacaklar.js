...
function taksitPlan(k){
  ...
  for(var i=0;i<k.taksitSayisi;i++){
    plan.push({ay:ayEkle(k.basTarih,i),tutar:k.tutar/k.taksitSayisi,no:i+1,odendi:(k.odemeler&&k.odemeler[i])||false});
  }
  return plan;
}
...
function render(){
  ...
  if(!_kayitlar.length){
    h+='<div class="al-bos"><div style="font-size:40px;margin-bottom:12px">&#128184;</div><div>Henüz alacak kaydı yok</div></div>';
  } else {
...
function kartHtml(k){
  ...
  h+='<span style="color:var(--text-muted);font-size:11px"> · '+para(k.tutar)+' TL toplam</span>';
  h+='</div></div>';
  ...
  h+='<div class="al-kart-actions"><button class="al-duz-btn row-action-btn duzenle" data-id="'+k.id+'">&#9998;</button><button class="al-sil-btn row-action-btn sil" data-id="'+k.id+'">&#10005;</button></div>';
  h+='</div></div>';
  h+='<div class="al-plan">';
  plan.forEach(function(t){
    ...
    h+='<button class="al-odeme-btn'+(t.odendi?" al-odendi-aktif":"")+'" data-id="'+k.id+'" data-no="'+(t.no-1)+'">';
    h+=t.odendi?'&#10003; Odendi':'Odendi işaretle';
    h+='</button></div>';
  });
  h+='</div></div>';
  return h;
...
function modalHtml(){
  ...
  h+='<div class="field-group"><label class="field-label">Açıklama (isteğe bağlı)</label><input type="text" id="al-aciklama" class="field-input" placeholder="Konu, sebep..." maxlength="100"/></div>';
  h+='<div class="field-group"><label class="field-label">Odeme Tipi</label>';
  h+='<div class="al-tip-secici"><button class="al-tip-btn active" data-tip="pesin">&#128184; Peşin</button><button class="al-tip-btn" data-tip="taksit">&#128200; Taksitli</button></div></div>';
  ...
  h+='<div class="field-group"><label class="field-label">Taksit Sayısı</label><input type="number" id="al-taksit-sayi" class="field-input" value="1" min="1" max="60"/></div>';
...
function modalAc(id){
  ...
  if(id){
    ...
    if(k){
      $("al-kisi").value=k.kisi;$("al-tutar").value=k.tutar;$("al-aciklama").value=k.aciklama||"";
      _aktifTip=k.tip;
...
async function kaydet(){
  ...
  if(_aktifTip==="pesin"){
    ...
    var ts=parseInt($("al-taksit-sayi").value)||1;
    var bt=$("al-bas-tarih").value;if(!bt){alert("Başlangıç ayı giriniz.");return;}
    kayit={tip:"taksit",kisi:kisi,tutar:tutar,aciklama:aciklama,taksitSayisi:ts,basTarih:bt,odemeler:{}};
  }
  if(_aktif){
