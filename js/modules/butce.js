...
var ButceModule = (function() {
  ...
  var YAPI=[
    {bolum:"gelir",baslik:"GEL\u0130R",satirlar:[
      ...
      {id:"toplam_maas",label:"TOPLAM MAA\u015e",hesaplanan:true,hesapla:function(d){return (d.salim_maas||0)+(d.bugra_maas||0);}},
    ...
    {bolum:"zorunlu",baslik:"ZORUNLU G\u0130DERLER",satirlar:[
      ...
      {id:"saglik",label:"SA\u011eLIK"},{id:"zekat",label:"ZEKAT"},
    ...
    {bolum:"istege",baslik:"\u0130STE\u011eE BA\u011eLI G\u0130DERLER",satirlar:[
      {id:"eglence",label:"E\u011eLENCE-YEMEK-E\u011e\u0130T\u0130M"},{id:"cocuk",label:"\u00c7OCUK"},
      {id:"giyim",label:"G\u0130Y\u0130M"},{id:"kredi_kart_ev",label:"KRED\u0130 KARTI"},{id:"oyle",label:"\u00d6YLE"},
  ...
  function hesapla(){YAPI.forEach(function(b){b.satirlar.forEach(function(s){if(s.hesaplanan)_veri[s.id]=s.hesapla(_veri);});});}
  ...
  async function veriYukle(){
    ...
    if(typeof window._fbDb!=="undefined"&&window._fbDb){
      try{var snap=await window._fbDb.ref(key).once("value");var d=snap.val()||{};_veri=d.veri||{};_ozelSatirlar=d.ozelSatirlar||{};}catch(e){}
    }
  }
  async function veriKaydet(){
    var key="butce_"+_aktifYil+"_"+(_aktifAy+1);
    if(typeof window._fbDb!=="undefined"&&window._fbDb){
      try{await window._fbDb.ref(key).set({veri:_veri,ozelSatirlar:_ozelSatirlar});}catch(e){}
    }
  }

  function satirEkle(bolum){
    var label=prompt("Yeni satir adi:");if(!label||!label.trim())return;
    ...
    var id=uid();_ozelSatirlar[bolum].push({id:id,label:label.trim().toUpperCase(),sira:max+1});
    _veri[id]=0;veriKaydet();render();
  }
  function satirSil(bolum,id){
    ...
    delete _veri[id];veriKaydet();render();
  }
  function satirRename(bolum,id){
    var s=(_ozelSatirlar[bolum]||[]).find(function(x){return x.id===id;});if(!s)return;
    var yeni=prompt("Yeni ad:",s.label);if(!yeni||!yeni.trim())return;
    s.label=yeni.trim().toUpperCase();veriKaydet();render();
  }
  ...
  function baglaOrtalama(bolum){
    ...
    rows.forEach(function(row){
      ...
      row.addEventListener("drop",function(e){
        e.stopPropagation();this.classList.remove("bt-drag-over");if(_dragSrc===this)return;
  ...
  function buildRow(satir,gelir,isOzel,bolum){
    ...
    h+='<td class="bt-col-sira">'+(isOzel?'<span class="bt-drag-handle">&#8597;</span>':'')+'</td>';
  ...
  function render(){
    ...
    h+='<button class="butce-rapor-btn" id="butce-rapor-btn">&#8595; CSV</button>';
    h+='</div>';
    ...
    YAPI.forEach(function(bolum){
      ...
      var topSatir=null;
      bolum.satirlar.forEach(function(s){if(s.label==="TOPLAM"){topSatir=s;return;}h+=buildRow(s,gelir,false,bolum.bolum);});
  ...
  function guncelle(){
    hesapla();var gelir=toplamGelir(),harcanan=toplamHarcanan(),kalan=gelir-harcanan;
    ...
    document.querySelectorAll(".bt-input").forEach(function(inp){var pctEl=inp.closest("tr").querySelector("[data-pct]");if(pctEl)pctEl.textContent=pct(parseFloat(inp.value)||0,gelir);});
