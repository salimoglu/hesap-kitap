...
const IslemlerDB = {
  ...
  async add(islem) { await openDB(); return promisify(tx(STORES.ISLEMLER, 'readwrite').add({ ...islem, olusturma: Date.now() })); },
  async update(islem) { await openDB(); return promisify(tx(STORES.ISLEMLER, 'readwrite').put(islem)); },
  async delete(id) { await openDB(); return promisify(tx(STORES.ISLEMLER, 'readwrite').delete(id)); },
...
const KategorilerDB = {
  ...
  async add(kat) { await openDB(); return promisify(tx(STORES.KATEGORILER, 'readwrite').add(kat)); },
